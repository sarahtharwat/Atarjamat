...
function corpo_enqueue_scripts()
{
    if (!is_admin()) {
        ...
        wp_register_script('custom-scripts', get_stylesheet_directory_uri() . '/js/scripts.js', array(), '1.0.0'); 
...
function corpo_enqueue_css()
{
    ...
    wp_register_style('color_scheme', get_stylesheet_directory_uri() . '/css/color_scheme/'.$color_scheme.'.css');
...
function corpo_bottom_sidebar_params($params) {

    global $corpo_widget_num; // Global a counter array
    ...
    if ( $sidebar_id == 'homepage-sidebar' || $sidebar_id == 'prefooter-sidebar' ) {
    ...
    if ( count($registered_widgets[$sidebar_id]) > 4 ) {
        $number_of_widgets = 1;
    }else {    
        $number_of_widgets = count($registered_widgets[$sidebar_id]);
    }
        
    if(!$corpo_widget_num) {// If the counter array doesn't exist, create it
        $corpo_widget_num = array();
    }

    if(isset($corpo_widget_num[$sidebar_id])) { // See if the counter array has an entry for this sidebar
        $corpo_widget_num[$sidebar_id] ++;
    } else { // If not, create it starting with 1
        $corpo_widget_num[$sidebar_id] = 1;
    }

    $classes = 'one-' . $width[$number_of_widgets];

    if($corpo_widget_num[$sidebar_id] == 1) { // If this is the first widget
        $classes .= ' first ';
    } elseif($corpo_widget_num[$sidebar_id] == count($registered_widgets[$sidebar_id])) { // If this is the last widget
        $classes .= ' last ';
    }
...
add_filter('dynamic_sidebar_params','corpo_bottom_sidebar_params');
